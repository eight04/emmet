...
define(function(require, exports, module) {
	...
	function createMatcher(text, pos) {
		...
		Range.prototype.extend = function(range){
			l = getLeftPair, the right most range that l.start < range.start
			r = getRightPair, the left most range that r.end > range.end
			if (!l) {
				if (!r) {
					cover all ranges
				} else {
		...
		var Match = function(text, startPos, token, name, excludeName) {
			this.i = startPos;
			this.token = token;
			this.name = name;
			this.excludes = [];
		...
		var Tree = function(matchGroup, searchMethod, root) {
		...
		Tree.prototype.exclude = function(){};

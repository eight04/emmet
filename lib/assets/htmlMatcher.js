	...
	function createMatcher(text, pos) {
		...
		function createForwardTagTree(root) {
			...
			function select() {
				...
				if (open.tag) {
					if (close.tag) {
						select = open.tag.range.start <= close.tag.range.start ? open : close;
					} else {
						select = open;
					}
				} else if (close.tag) {
			...
			return {
				next: function(){
					...
					var tag = select(), detached, finished = false;
					if (tag) {
						if (tag.type == "open") {
							stack.push(tag);
						} else {
		...
		function createForwardSearch() {
			...
			return {
				next: function() {
					...
					if (result.tag.type == "close" && !result.detached) {
						this.tag = result.tag;
						this.finished = true;
						return;
					}

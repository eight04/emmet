...
define(function(require, exports, module) {
	...
	Search.prototype.createTreeNext = function(tag){
		this.excludeRange = this.main.getExclude();
		return new Tree(
			new MatchGroup([
				new Match(this.text, tag.range.end, "<" + tag.name, tag.name, this.excludeRange),
				new Match(this.text, tag.range.end, "</" + tag.name, tag.name, this.excludeRange),
			]),
			this.direction,
			tag
	...
	Search.prototype.createTreeBack = function(tag) {
		this.excludeRange = this.main.getExclude();
		return new Tree(
			new MatchGroup([
				new Match(this.text, tag.range.start - 1, "<" + tag.name, tag.name, this.excludeRange),
				new Match(this.text, tag.range.start - 1, "</" + tag.name, tag.name, this.excludeRange),
			]),
			this.direction,
			tag
	...
	Search.prototype.next = function(){
		...
		if (result.type == this.openType && !this.pool[result.tag.name]) {
			this.pool[result.tag.name] = this.createTree(result.tag);
			this.trees.push(result.tag.name);
		}
		
		var i = this.trees.length, name, range;
		while (i) {
			name = this.trees[i - 1];
	...
	Search.prototype.keepMatchingPair = function(tag) {
		this.tag = null;
		this.finished = false;
	...
	var Range = function(start, end, prev, next) {
		this.start = start;
		this.end = end;
		this.next = null;
		this.prev = null;
	...
	Match.prototype.back = function(){
		...
		do {
			...
			// var len = ex.length;
			// jump over
			while (ex.prev && ex.prev.start >= this.i) {
				ex = ex.prev;
			}
			this.excludeRange = ex;
			// while (len && this.i < ex[len - 1].start) {
				// ex.pop();
				// len--;
			// }
			// jump to end
			...
			// if (len && this.i < ex[len - 1].end) {
				// ex.pop();
				// this.i = ex[len - 1].start - 1;
				// continue;
			// }
	...
	Match.prototype.next = function() {
		...
		do {
			...
			// jump over
			while (ex.next && ex.next.end <= this.i) {
				ex = ex.next;
			}
			this.excludeRange = ex;
	...
	Match.prototype.addExclude = function(range){
		...
		// var i, len;
		// for (i = 0, len = this.excludes.length; i < len; i++) {
			// if (this.excludes[i].end >= range.start) {
				// break;
			// }
		// }
		// var j;
		// for (j = i; j < len; j++) {
			// if (range.end < this.excludes[j].start) {
				// break;
			// }
		// }
		// range = new Range(
			// this.excludes[i] ? Math.min(range.start, this.excludes[i].start) : range.start,
			// this.excludes[j - 1] ? Math.max(range.end, this.excludes[j - 1].end : range.end)
		// );
		// this.excludes.splice(i, j - i, range);
	};
	
	/* Match.prototype.excludeNext = function(range){
		if (this.i < 0) {
			return;
		}
		if (this.i < range.start) {
			this.addExclude(range);
		} else if (this.i < range.end) {
			search.i = range.end;
		}
	};
	
	Match.prototype.excludeBack = function(range){
		if (this.i < 0) {
			return;
		}
		if (range.end <= this.i) {
			// not search yet, push to excludes
			this.addExclude(range);
		} else if (range.start <= this.i) {
			// in exclude range
			this.i = range.start - 1;
		}
	}; */
	...
	Tree.prototype.next = function(){
		...
		var tag = this.matchGroup[this.direction](), result;
	...
	function createMatcher(text, pos) {
		...
		function buildExclude() {
			var i = 0, range = excludeRange = new Range(pos, pos);
		...
		return {
			...
			loop: function() {
				...
				if (backward.tag && backward.tag.name != forward.tag.name) {
					backward.keepMatchPair(forward.tag);
				}				

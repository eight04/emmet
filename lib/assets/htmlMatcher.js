...
define(function(require, exports, module) {
	...
	function matchTag(text, pos) {
		...
		if (text[pos + 1] == "/") {
			...
			if ((match = text.indexOf("-->", pos + 4)) >= 0) {
				return comment(pos, match);
			} else {
				return -1;	// unfound
			}
		} else if (match = text.substr(pos).match(start_tag)) {
			// open
			return tag(match, pos, "start");
		}
	}
	...
	function createMatcher(text, pos) {
		...
		return {
			...
			searchForward: function(){
				var match;
				while ((match = forward()) != -1) {
					if (!match) {
						continue;
					}

					if (handleF[match.type]) {
						if (foundF = handleF[match.type](match)) {
							return;
						}
					}
				}
			...
			searchBackward: function(){
				var match;
				while ((match = backward()) != -1) {
					if (!match) {
						continue;
					}

					if (handleB[match.type]) {
						var temp = handleB[match.type](match);
						if (temp.name == foundF.name || temp.type == "comment") {
							foundB = temp;
							return;
						}
					}
			...
			// This should be faster if the pos is nearing top or bottom of the document.
			searchBoth: function(){
				var matchF, matchB;
				while ((matchF = forward()) != -1 && (matchB = backward()) != -1) {
					if (matchF && handleF[matchF.type]) {
						foundF = handleF[matchF.type](matchF);
					}
					if (matchB && handleB[matchB.type]) {
						var temp = handleB[matchB.type](matchB);
	...
	HTMLPairMatcher.find = function(html, start_ix) {
		var matcher = createMatcher(html, start_ix);
		matcher.searchBoth();
		var pair = matcher.result();

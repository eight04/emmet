	...
	function createMatcher(text, pos) {
		...
		function searchOpen(token, pos) {
			...
			if (mark[token][match] === undefined) {
				...
				if (tag && !tag.selfClose) {
					return match;
				}
			}
			mark[token][match] = searchOpen(token, mark[token][match] - 1);
		...
		return {
			...
			searchForward: function(){
				...
				if (hit) {
					...
					while (matchClose > 0) {
						...
						matchOpen = searchOpen(open, matchClose);
					...
					while (matchClose > 0) {
						match = end_tag_g.exec(text);
						if (!match) {
							return;
						}
						// matchClose = matchTag(match.index);
						matchOpen = searchOpen("<" + match[1], match.index);
						if (matchOpen < 0) {
							return;
						}
						if (matchOpen < pos) {
			...
			searchBackward: function(){
				...
				var matchOpen = searchOpen(open, foundF.range.start);
				if (matchOpen < 0) {
					return;
				}
				foundB = matchTag(matchOpen);
